                            ...
                            Bill.findOne({ _id: new ObjectId(req.query._id), payment_status: 0 }, async function (err, bill) {
                                if (err || !bill) res.status(400).send({ message: "Không tìm thấy id hóa đơn", data: "false" });
                                    ...
                                    for (let dish of bill.dishes) {
                                        line_items.push({
                                            name: dish.name,
                                            images: (dish.feature_image)?[dish.feature_image]: ["https://breakthrough.org/wp-content/uploads/2018/10/default-placeholder-image.png"],
                                    ...
                                    if (line_items.length != 0) {
                                        let session = await stripe.checkout.sessions.create({
                                            ...
                                            success_url: `https://cateye0609.github.io/partybooking/client/payment/success`,
                                            cancel_url: `https://cateye0609.github.io/partybooking/client/payment/cancel`,
