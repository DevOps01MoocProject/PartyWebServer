...
module.exports = {
	...
	add_dish: function (req, res) {
		MongoClient.connect(
			process.env.DATABASE_URL || 'mongodb://cateye02:madeinabyss123@cluster0-shard-00-00.jwldl.mongodb.net:27017,cluster0-shard-00-01.jwldl.mongodb.net:27017,cluster0-shard-00-02.jwldl.mongodb.net:27017/PartyBooking?ssl=true&replicaSet=atlas-7jugia-shard-0&authSource=admin&retryWrites=true&w=majority',
			function (err, db) {
	...
	get_dish: function (req, res) {
		MongoClient.connect(process.env.DATABASE_URL || 'mongodb://cateye02:madeinabyss123@cluster0-shard-00-00.jwldl.mongodb.net:27017,cluster0-shard-00-01.jwldl.mongodb.net:27017,cluster0-shard-00-02.jwldl.mongodb.net:27017/PartyBooking?ssl=true&replicaSet=atlas-7jugia-shard-0&authSource=admin&retryWrites=true&w=majority',
			function (err, db) {
				var Menu = db.collection("Menu");
	...
	statistic_dish: function (req, res) {
		MongoClient.connect(process.env.DATABASE_URL || 'mongodb://cateye02:madeinabyss123@cluster0-shard-00-00.jwldl.mongodb.net:27017,cluster0-shard-00-01.jwldl.mongodb.net:27017,cluster0-shard-00-02.jwldl.mongodb.net:27017/PartyBooking?ssl=true&replicaSet=atlas-7jugia-shard-0&authSource=admin&retryWrites=true&w=majority', function (err, db) {
			var Bill = db.collection("Bill");
	...
	upload_image: function (req, res) {
		if (req.files && req.files.length != 0) {
			...
			for (let index of req.files) {
				array_image.push("http://20.37.98.82:49160/open_image?image_name=" + index.filename)
			}
			if (req.body.type)
		...
		MongoClient.connect(
			...
			function (err, db) {
				...
				if (req.body.title) {
					...
					req.body.link = "http://20.37.98.82:49160/client/post/detail/"+req.body.link
					Posts.findOne({ link: req.body.link},function (err, data) {
						if (!data && !err) {
							if (!req.body.content) req.body.content = "";
							if (!req.body.type) req.body.type = "post";
							if (!req.body.feature_image) req.body.feature_image = "https://breakthrough.org/wp-content/uploads/2018/10/default-placeholder-image.png";
							req.body.author = req.body.username;
							req.body.create_at = new Date();
							req.body.update_at = new Date();
							delete req.body.username;
		...
		MongoClient.connect(
			...
			function (err, db) {
				...
				if (req.params.id) {
					...
					else Posts.findOne({ link: "http://20.37.98.82:49160/client/post/detail/" + req.params.id},function (err, data) {
						if (!err && data) {
							res.status(200).send({message: "Lấy nội dung bài viết", data: data});
						}
						else res.status(400).send({message: "Không tìm thấy Id bài viết",data: "false"});
